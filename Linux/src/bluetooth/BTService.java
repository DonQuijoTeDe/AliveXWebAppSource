...
public class BTService {
    /*
     *1 If you want to connect to the mobile device to do
     *  some testing, please change the uuidSet to HANDSFREE
     *2 If you want to connect to the AliveSensor, please
     *  change the uuidSet to SERIAL_PORT
     */
    private Object completedEvent;
    private UUID SERIAL_PORT = new UUID(0x1101);
    private UUID HANDSFREE = new UUID(0x111E);
    private UUID[] uuidSet = new UUID[] { HANDSFREE };
    ...
    public void getService() {
        try {
            ...
            for(int i = 0; i < remoteDevices.size(); i++) {
                ...
                System.out.println(btname+" "+btaddress+" "+remoteDevices.size());

                synchronized (completedEvent) {
                    LocalDevice.getLocalDevice().getDiscoveryAgent().searchServices(
                                attrIDs, uuidSet, remoteDevices.get(i), listener);
